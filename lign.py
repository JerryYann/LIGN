...
class net(nn.Module):
    def __init__(self):
        ...
        self.fe = FE(13, 32, 32, 32, 32)  # 13 for gray, 39 for color
        ...
        self.de11 = Decoder(128, 64)  # 64
        self.tail = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, stride=1, padding=1)  # 1 for gray
    ...
    def forward(self, x):
        ...
        res = self.tail(de11) + x[:,0:1,:,:]
        # + x[:,0:3,:,:] for color

        return sp, res

if __name__ == '__main__':
    inp = torch.randn((1, 13, 128, 128))
    net = net()
    sp,out = net(inp)
    print(out.shape)